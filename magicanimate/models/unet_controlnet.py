...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def from_pretrained_2d(cls, pretrained_model_path, subfolder=None, unet_additional_kwargs=None):
        ...
        original_path = pretrained_model_path  # Store for later use
        ...
        # Handle model file loading for both local and HuggingFace paths
        model_file = os.path.join(pretrained_model_path, WEIGHTS_NAME)
        if not os.path.isfile(model_file):
            # Try .safetensors format
            safetensors_file = os.path.join(pretrained_model_path, "diffusion_pytorch_model.safetensors")
            if os.path.isfile(safetensors_file):
                ...
                try:
                    weight_path = (subfolder + "/" if subfolder else "") + "diffusion_pytorch_model.bin"
                    ...
                    # Try safetensors
                    weight_path = (subfolder + "/" if subfolder else "") + "diffusion_pytorch_model.safetensors"
                    model_file = hf_hub_download(
                        repo_id=original_path,
                        filename=weight_path,
                        repo_type="model"
